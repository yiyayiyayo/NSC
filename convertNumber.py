...
def base64_decode(text, base, alphabet=ALPHA):
...
if len(sys.argv) == 4 and sys.argv[3] != "1":
    ...
    # create dictionary to create xml from it
    decimalDic = dict(title=str(decimal), subtitle="Decimal", uid="decimal", valid=True, arg=str(decimal),
    ...
    if len(sys.argv) != 4:
        errorDic = dict(title="Make sure to pass 3 numbers",
                        subtitle="convert `number` `base of original` `base to convert to`", uid="error", valid=False,
                        arg="error")
